...
std::vector<Point2D> Catcher::returnNeighbors(Point2D pos) {
...
bool Catcher::neighborExists(std::vector<Point2D> list, Point2D pos) {
  for (int i = 0; i < list.size(); i++) {
    if (list[i].x == pos.x && list[i].y == pos.y) return true;
  }
  return false;
}